...
local function wczytajStan()
  -- table.save(path, t) serializuje t jako element listy varargs; table.load
  -- bez tabeli docelowej zwraca pierwszy element (jak w ishtar_cal).
  local ok, t = pcall(table.load, STAN_PATH)
...
function gui.odswiez()
  ...
  if puste then
    setLbl("trng.chip.mz", " 0 mdz ")
    showLbl("trng.chip.mz", true)
  end

  setLbl("trng.razem", "razem: " .. formatujLiczbe(wynik.miedziRazem) .. " mdz")
...
function gui.render()
  ...
  txt("trng.wybrana", 310, 4, 300, 24, "", gui.win, STYLE.lbl)
  ...
  txt("trng.chip.mth", 10, 470, 140, 24, "", gui.win, STYLE.mth)
  txt("trng.chip.zl", 158, 470, 140, 24, "", gui.win, STYLE.zl)
  txt("trng.chip.sr", 306, 470, 140, 24, "", gui.win, STYLE.sr)
  txt("trng.chip.mz", 454, 470, 156, 24, "", gui.win, STYLE.mz)
  txt("trng.razem", 10, 498, 300, 16, "", gui.win, STYLE.lbl)
...
local function updateCacheRead()
  local ok, t = pcall(table.load, UPDATE_CACHE_PATH)
