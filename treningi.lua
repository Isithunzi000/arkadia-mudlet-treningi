...
-- Weryfikacja pobranej paczki PRZED odinstalowaniem starej wersji
-- (uszkodzony download nie moze zostawic gracza bez dzialajacego
-- pluginu). Czysty Lua, bez zaleznosci, cross-platform:
-- magic PK\3\4 + rekord koncowy EOCD PK\5\6 + nazwa wlasnego XML
-- w niekompresowanych naglowkach zipa.
local UPDATE_OWN_XML = "treningi.xml"
local function updateVerifyPkg(path)
  local f = io.open(path, "rb")
  if not f then return false end
  local body = f:read("*a")
  f:close()
  if type(body) ~= "string" then return false end
  if body:sub(1, 4) ~= "PK\003\004" then return false end
  if body:find("PK\005\006", -65557) == nil then return false end
  if body:find(UPDATE_OWN_XML, 1, true) == nil then return false end
  return true
end

local function updateOnPkgDone(event, path)
  if path ~= updatePkgPath then return end
  if not updateVerifyPkg(path) then
    updateKillHandlers()
    updatePkgPath = nil
    updatePendingVersion = nil
    pcall(os.remove, path)
    updateFail("pobrania paczki")
    return
  end
  updateKillHandlers()
